...
class TradfriRemoteDimmer(hass.Hass):
  ...
  def initialize(self):
    ...
    self.dim_action = 0
    self.target_remote_type = self.args['remote']['id']
    self.log(f"Initialized dimming to IKEA tradfri remote control [{self.target_remote}] type [{self.target_remote_type}] on target lights [{self.target_light}]")
